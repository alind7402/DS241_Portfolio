...
census_api_key("52229c4963bc0dd843a8f8ce5c0dfb1406ee25de")
...
```{r}
df_daytime_p = get_acs(geography = "county",
                  variables = c("workers_tot" = "B08604_001",
                                "resident_pop" = "B01003_001",
                                "workers_living_dc" = "B08009_001"),
                  state="DC",geometry=TRUE,year=2018, cache_table = TRUE) %>%
  pivot_wider(id_cols = c("GEOID", "NAME", "geometry"),
              names_from = "variable", values_from = "estimate") %>%
  mutate(daytime_population = resident_pop + workers_tot - workers_living_dc)
...
```{r}
df_daytime_p  %>% tm_shape()+tm_polygons(c("resident_pop","allocation_of_workers","sex_of_workers"),alpha=.4)
```
